# --- START: CHANGED FOR METHOD COMPLETION
set -l methods get head post put delete connect options trace patch
set -l example_url https://example.com/
complete -c xh -f -n "not __fish_seen_subcommand_from $methods" -a "$methods"
complete -c xh -f -n "__fish_seen_subcommand_from $methods; and not __fish_seen_subcommand_from $example_url" -a "$example_url"
complete -c xh -f -n "__fish_seen_subcommand_from $methods $example_url"
# --- END  : CHANGED FOR METHOD COMPLETION
