...
#let report(
  ...
  show raw.where(block: true): it => locate(loc => {
    ...
    set figure.caption(position: top, separator: if cur-code-info.caption == none { none } else { auto })
    ...
    let code-block-content = grid(
      ...
      ..it
        ...
        .map(line => {
          ...
          let line-number-color = if show-added { rgb("#228b22") } else if show-deleted { rgb("b22222") } else { gray }
          let line-bg-color = if show-highlighted { rgb("#cfecfc") } else if show-added {
            rgb("#d6f2c7")
          } else if show-deleted { rgb("#ffd9d9") } else { rgb(0, 0, 0, 0) }

          let line-number-content = grid.cell(inset: (right: 1.2em), text(
    ...
    let code-block = [
      ...
      #figure(
        kind: raw,
        caption: if cur-code-info.caption == none {
          if cur-code-info.label == none { none } else { "" }
        } else {
          cur-code-info.caption
        },
        supplement: "コード",
        numbering: if cur-code-info.label == none { none } else { "1" },
        gap: 1em,
        {
